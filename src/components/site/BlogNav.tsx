...
const Behance = ({ size = 24, ...props }: any) => (
  <svg width={size} height={size} {...props} viewBox="0 0 16 16" fill="currentColor">
...
const BlogNav = memo(() => {
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-[padding,background-color,backdrop-filter] duration-500 transform-gpu will-change-[padding,background-color,backdrop-filter] gpu-layer ${
        scrolled ? "py-3 backdrop-blur-sm bg-paper/80 border-b border-ink/10" : "py-6 bg-transparent"
      }`}
    >
        ...
        <ul className="hidden md:flex items-center gap-10 font-mono-tag text-ink-soft">
          {links.map((l) => (
            <li key={l.href}>
              <Link
                href={l.href}
                className={`link-underline hover:text-ink ${l.href === "/blog" ? "text-teal" : ""}`}
              >
                {l.label}
              </Link>
            </li>
      ...
      {open && (
        <div id="mobile-menu-blog" className="md:hidden border-t border-ink/10 bg-paper">
